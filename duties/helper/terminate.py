...
class GracefulTerminator:
    ...
    def __init__(self, max_number_of_cicd_rounds: int) -> None:
        self.kill_now = False
        self.__round_counter = 0
        self.__max_number_of_cicd_rounds = max_number_of_cicd_rounds
    ...
    def terminate_in_cicd_mode(
        ...
        match running_mode:
            ...
            case Mode.CICD_WAIT:
                if len(duties) == 0:
                    sys_exit(0)
                if self.__round_counter >= self.__max_number_of_cicd_rounds:
                    sys_exit(1)
